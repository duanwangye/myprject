...
class User extends Base
{
    public function checkUserByMobile() {
        ...
        if(!$model) {
            $isTongbu = (new Tongbu())->DY_user($this->app['mobile']);
            if(!$isTongbu) {
    ...
    public function checkInfo() {
        ...
        */
        ...
        if($this->app['payword'] != $this->app['paywordRe']) {
            return Common::rm(-1, '两次输入密码不一致');
        }

        if(!Validate::is($this->app['payword'],'number')) {
            return Common::rm(-1, '支付密码必须为数字');
        }
        ...
        if(false) {
            ...
            $user = $data['user'];
            $this->app['trueName'] = $user->getTrueName();
            $this->app['mobile'] = $user->getMobile();
            $this->app['passport'] = $user->getCardNumber();
            $this->app['bankNumber'] = $user->getBankNumber();
            $this->app['bankName'] = $user->getBankName();
        }
        else {
            $user = new \User();
    ...
    public function getUserInfo() {
        ...
        $item->userAccount = $item->userAccount->updateProfit();

        $item = $item->append(['userAccount'=>['moneyTotal']])->visible([
            'userID','mobile','avatar','gender','passport','trueName','loginTime','isAuthTrueName','isAuthBank','isNewInvest','userAccount'=>[
                'money','moneyAcc','moneyYesterday','moneyToday','waitBen','waitInterest','hasInvestBenTotal','hasInvestMoneyTotal','hasRepayBenTotal','hasRepayInterestTotal'
            ]
        ])->toArray();
        ...
        $bankF = Bank::get($bankDefault['bankID']);
