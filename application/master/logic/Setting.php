...
class Setting extends Base
{
    ...
    public function channelGetDetail() {
        $item = Channel::get($this->app['channelID']);
        if(!$item) {
            return Common::rm(-32, '没有数据');
        }
        return Common::rm(1, '操作成功', [
    ...
    public function channelUpdate() {
        if(!isset($this->app['channelID']) ||!$this->app['channelID']) {
            ...
            $model['channelID'] = $this->app['channelID'];
        }
    ...
    public function channelGetList() {
        ...
        if(isset($this->app['keyword']) && $this->app['keyword']) {
            $map['name|code'] = ['like', '%'.$this->app['keyword'].'%'];
        }
        ...
        $list = Channel::where($map)->order('addTime desc')->limit(($this->app['pageIndex'] - 1) * $this->app['pageItemCount'], $this->app['pageItemCount'])->order('addTime desc')->select();
        if($list->isEmpty()) {
            return Common::rm(-2, '数据为空');
        }
