...
class H5
{
    ...
    public function __construct()
    {
        ...
        if($this->track['osType'] == 1) {
            $this->skin = Config::get('skin_channel_'.$this->track['channel'].'.app_skin_config');
        }
        Log::info($this->app);
    ...
    public function contract() {
        return view(__FUNCTION__, [
            'resourcePath'=>'/static/'.request()->module().'/'.strtolower(request()->controller()).'/'.'contract',
    ...
    public function userRecharge() {
        ...
        $recharge->setPageUrl($this->h5RootUrl().'/notify/fuyou/rechargePage/userRechargeID/'.$userRecharge['userRechargeID']);
    ...
    public function userRechargeWY() {
        ...
        $recharge->setMoney($this->app['money']);
        $recharge->setLoginID($userBank['mobile']);
        $recharge->setPageUrl($this->h5RootUrl().'/notify/fuyou/rechargeWYPage/userRechargeID/'.$userRecharge['userRechargeID']);
        $recharge->setNotifyUrl($this->h5RootUrl().'/notify/fuyou/rechargeWYNotify/userRechargeID/'.$userRecharge['userRechargeID']);
        $data = (new Finance())->userRechargeWY($msg, $recharge);
    ...
    public function userDrawcash() {
        ...
        $drawcash->setPageUrl($this->h5RootUrl().'/notify/fuyou/drawcashPage/userDrawcashID/'.$userDrawcash['userDrawcashID']);
    ...
    public function userCancelUser() {
        $finance = new Finance();
        //$user = Model::get($this->app['userID']);

        $result = $finance->userCancel($msg, $this->user['mobile'], $this->h5RootUrl().'/notify/fuyou/cancelUser');
        UserCancelLog::create([
            'userID'=>$this->user['userID'],
            'outerNumber'=>$result['outerNumber'],
            'status'=>UserCancelLog::STATUS_UN,
            'outerAccount'=>$this->user['mobile'],
            'note'=>''
        ]);
        return view(__FUNCTION__, [
            'resourcePath'=>'/static/'.request()->path(),
    ...
    public function about() {
        return view(__FUNCTION__, [
                'resourcePath'=>'/static/'.request()->module().'/'.strtolower(request()->controller()).'/'.'about',
                'companyInfo'=>$this->skin['companyInfo']
            ]
        );
    }
    ...
    public function regAgreement() {
        return view(__FUNCTION__, [
            'resourcePath'=>'/static/'.request()->module().'/'.strtolower(request()->controller()).'/'.'regAgreement',
            'companyInfo'=>$this->skin['companyInfo']
        ]);
    }
    ...
    public function userInvitationPage() {
        ...
        Log::info($this->data);
        $from = 0;
        if(isset($this->data['from']) && $this->data['from'] == 1) {
            $from = 1;
        }
        return view(__FUNCTION__, [
            'resourcePath'=>'/static/'.request()->module().'/'.strtolower(request()->controller()).'/'.'userInvitationPage',
            'userInfo'=>$user,
            'from'=>$from
        ]);
    }
